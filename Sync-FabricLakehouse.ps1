...
function Build-AzCopyArgs {
    ...
    # Primary verb — mirror mode runs as 'copy' under the hood
    $actualVerb = if ($RunMode -eq "mirror") { "copy" } else { $RunMode }
    $args.Add($actualVerb)
    ...
    # sync-only flags
    if ($RunMode -eq "sync") {
        # Remove destination files that no longer exist at source
...
function Start-LakehouseSync {
    ...
    # ── MIRROR mode: copy all files then delete extras at destination ──
    # Ideal for Delta tables where UPDATEs create new files that 'sync'
    # may miss due to OneLake caching of Last-Modified-Time.
    if ($Mode -eq "mirror") {
        Write-StructuredLog -Level "INFO" -RunMode "mirror" -Command "" `
            -Message "Mirror mode: copying all files (overwrite=true), then cleaning up destination."

        # Pass 1: copy all files from source to dest
        $result = Invoke-AzCopyWithRetry -RunMode "mirror"
        if ($result -ne 0) { exit $result }

        # Pass 2: sync --delete-destination to remove files that no longer exist at source
        # (e.g., parquet files vacuumed from the Delta table)
        Write-StructuredLog -Level "INFO" -RunMode "mirror" -Command "" `
            -Message "Mirror mode pass 2: deleting destination-only files via sync."
