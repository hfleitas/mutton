...
# ─────────────────────────────────────────────────────────────────────────────
$RunMode = "sync"           # Change to "copy" for the very first run
$ContinuousSync = $false    # Set to $true for an ongoing loop
$IntervalMinutes = 5

foreach ($pair in $SyncPairs) {
    Write-Host "`n>>> Syncing: $($pair.Source) -> $($pair.Dest)" -ForegroundColor Cyan
    ...
    if ($ContinuousSync -and $RunMode -eq "sync") {
